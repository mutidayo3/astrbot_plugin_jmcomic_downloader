...
- 💬 **消息自动撤回**：状态提示 30 秒后自动撤回，群聊中最终只留下文件
...
| `auto_recall` | 布尔 | `true` | 自动撤回状态消息，30 秒后撤回，群聊中最终只留下文件 |
...
| 版本 | 更新内容 |
|------|----------|
| 0.0.29 | **消息自动撤回 + 健壮性修复**：<br>- 新增 `_message_manager.py` 模块，通过 OneBot API 直接发送消息并追踪 message_id<br>- 新增 `auto_recall` 配置项（默认开启），状态消息 30 秒后自动撤回，群聊最终只留文件<br>- 多条撤回间隔 ≥6 秒，避免触发平台风控<br>- 错误消息（❌）和密码提示（🔐）不会被撤回<br>- 修复 `sync_disk_to_memory` 遗漏 JM 前缀文件的问题<br>- 新增 `_cleanup_dirty_files()` 启动清理：清除异常重载遗留的临时目录和废 PDF<br>- `terminate()` 统一清理下载子进程（SIGTERM → SIGKILL），防止重载后进程残留<br>- 限频检查加 asyncio.Lock 保证原子性，yield 移出锁外防止死锁 |
